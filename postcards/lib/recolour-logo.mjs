...
//
// Pass --wordmark to leave the orange mark alone and recolour only the
// wordmark, which is how the logo survives a dark background:
//
//   node lib/recolour-logo.mjs rowbo-logo.png rowbo-logo-light-mark.png "#FFFFFF" --wordmark
...
const wordmarkOnly = process.argv.includes('--wordmark');
if (!src || !out || !hex) {
...
const png = await page.evaluate(async ({ dataUrl, hex, wordmarkOnly }) => {
  const img = new Image();
  img.src = dataUrl;
  await img.decode();
  ...
  for (let i = 0; i < px.length; i += 4) {
    ...
    // The mark is rowbo orange, the wordmark is near-neutral ink; a red cast
    // this strong only ever comes from the mark.
    if (wordmarkOnly && px[i] > px[i + 1] + 40) continue;
...
await browser.close();
console.log(`${src} -> ${out} (${hex}${wordmarkOnly ? ', wordmark only' : ''})`);
